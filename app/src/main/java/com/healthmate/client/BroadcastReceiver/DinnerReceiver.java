...
public class DinnerReceiver extends BroadcastReceiver{
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        builder.setContentTitle("Dinner").setContentText("Dinner time🍗")
        ...
        String channelId = "dinner_notification";
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(channelId, "Dinner",NotificationManager.IMPORTANCE_DEFAULT);
            assert nm != null;
            nm.createNotificationChannel(channel);
            builder.setChannelId(channelId);
        }
        assert nm != null;
        nm.notify(5,notificationCompat);
